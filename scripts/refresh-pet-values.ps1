...
$summary += "- Tracker-backed lanes now update the broad long-tail value catalog."
...
$summary += ""
$summary += "- Publish benchmark pet changes only when the benchmark sync and divergence guardrails pass."
$summary += "- Cross-check benchmark and spotlight pets against a second market reference when sources disagree materially."
$summary += "- If audit-only mode was used, review the candidate files in `data/value-sync-staging` before publishing."
$summary += "- Only publish calculator override changes after QA passes and the conflict queue is clear."
